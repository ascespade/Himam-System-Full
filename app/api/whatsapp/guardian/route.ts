  ...
  try {
    ...
    if (!guardian) {
      await sendTextMessage(
        from,
        "عذراً، أنت غير مسجل كولي أمر في النظام. يرجى التواصل مع الإدارة.",
      );
      return NextResponse.json(successResponse({ action: "not_guardian" }));
    }

    // Get guardian's patients
    const relationships = await guardianRepository.getGuardianPatients(
      guardian.id,
    );
    ...
    if (existingConversation) {
      ...
      const { data: newConversation } = await supabaseAdmin
        ...
        .select(
          "id, phone_number, patient_id, status, last_message_at, unread_count, created_at, updated_at",
        )
        .single();
    ...
    if (!conversation) {
      return NextResponse.json(
        { success: false, error: "Failed to create or find conversation" },
        { status: 500 },
      );
    }
